...
**** Themes: ~solarized~, ~vscode-dark-plus~, ~waher~
I quite like the dark VSCode and Solarized themes, but ~waher~ is a personal favorite, so we'll use that. All are installed, so it's easy to change them.
#+begin_src emacs-lisp
